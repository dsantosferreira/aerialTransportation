...
void DirectFlights::execute() {
    ...
    if ((database.getGraph().getNodes().find(airport_code)) != database.getGraph().getNodes().end()) {
        ...
        for(int i=0; i<3- to_string(page+1).length();i++) cout<<' ';
        cout<<"                                      \033[0m|"<<endl;
        cout << "|\033[40m-------------------------------------------------------------------------------------\033[0m|" << endl;
        ...
        for (int i=10*page;i<10*page+10;i++ ){
            ...
            string city=database.getAirports().find(Airport(e.destCode,"","","",0,0))->getCity();
            cout<<"|";
            if(i%2==0)cout<<"\033[47m"<<"\033[30m";
            else  cout<<"\033[100m";
            cout<<" "<<e.airlineCode<<"                     | "<<e.destCode<<'-'<<country<<'-'<<city;
            for(int i=0; i<53-country.length()-city.length();i++)cout<<" ";
            cout<<"\033[0m"<<"|"<<endl;
        }
        ...
        for(int i=0; i<10- to_string(airlines.size()).length();i++) cout<<' ';
        cout << "| Airports:"<<airports.size();
        for(int i=0; i<11- to_string(airports.size()).length();i++) cout<<' ';
        cout << "| Countries:"<<countries.size();
        for(int i=0; i<11- to_string(countries.size()).length();i++) cout<<' ';
        cout << "| Cities:"<<cities.size();
        for(int i=0;i<11- to_string(cities.size()).length();i++) cout<<' ';
        cout<<"\033[0m|"<<endl;
        cout << "|\033[40m_____________________________________________________________________________________\033[0m|" << endl;
        ...
        while(cond) {
            ...
            if(cond)cout << "\033[31mInvalid input! Please enter a valid input: \033[0m";
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }}}else{
        cout << "\033[31m" <<"Airport not found!"<<"\033[0m"<<endl;
